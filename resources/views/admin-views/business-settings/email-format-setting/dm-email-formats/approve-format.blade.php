                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input" onclick="toogleStatusModal(event,'mail-status','place-order-on.png','place-order-off.png','{{translate('Want_to_enable_the')}} <strong>{{translate('New_Delivery_Partner_Approval')}}</strong> {{translate('mail')}} ?','{{translate('Want_to_disable_the')}} <strong>{{translate('New_Delivery_Partner_Approval')}}</strong> {{translate('mail')}} ?',`<p>{{translate('If_enabled,_Delivery_Partner_will_get_a_confirmation_email_when_registration_is_approved_by_the_Admin.')}}</p>`,`<p>{{translate('If_disabled,_Delivery_Partner_will_not_get_a_registration_approval_email.')}}</p>`)"  id="mail-status" {{$mail_status == '1'?'checked':''}}>
